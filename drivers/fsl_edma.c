...
void EDMA_SetTransferConfigRingBuffer(DMA_Type *base, uint32_t channel, const edma_transfer_config_t *config, uint32_t nextTcd)
{
    assert(channel < (uint32_t)FSL_FEATURE_EDMA_MODULE_CHANNEL);
    assert(config != NULL);

    EDMA_TcdSetTransferConfigRingBuffer((edma_tcd_t *)(uint32_t)&base->TCD[channel], config, nextTcd);
}
...
void EDMA_TcdSetTransferConfigRingBuffer(edma_tcd_t *tcd, const edma_transfer_config_t *config, uint32_t nextTcd)
{
    assert(tcd != NULL);
    assert(((uint32_t)tcd & 0x1FU) == 0U);
    assert(config != NULL);
    ...
    if (nextTcd != NULL)
    {
        ...
        */
        ...
        tcd->CSR = (tcd->CSR) & ~DMA_CSR_DREQ_MASK;        
    }
}
...
void EDMA_CreateHandleRingBuffer(edma_handle_t *handle, DMA_Type *base, uint32_t channel)
{
    assert(handle != NULL);
    assert(channel < (uint32_t)FSL_FEATURE_EDMA_MODULE_CHANNEL);
...
status_t EDMA_SubmitTransferRingBuffer(edma_handle_t *handle, const edma_transfer_config_t *config)
{
    assert(handle != NULL);
    assert(config != NULL);
    uint32_t tmpCSR = 0;
    ...
    edma_tcd_t *tcdRegs = (edma_tcd_t *)(uint32_t)&handle->base->TCD[handle->channel];
        ...
        */
        ...
        {
            EDMA_SetTransferConfigRingBuffer(handle->base, handle->channel, config, (uint32_t)(-(config->minorLoopBytes*config->majorLoopCounts)));
            /* Enable major interrupt */
            handle->base->TCD[handle->channel].CSR |= DMA_CSR_INTMAJOR_MASK;
...
void EDMA_StartTransferRingBuffer(edma_handle_t *handle)
{
    assert(handle != NULL);
