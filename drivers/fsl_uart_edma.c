...
status_t UART_ReceiveEDMARingBuffer(UART_Type *base, uart_edma_handle_t *handle, uart_transfer_t *xfer)
{
    assert(handle);
    assert(handle->rxEdmaHandle);
    assert(xfer);
    ...
    else
    {
        ...
        /* Submit transfer. */
        EDMA_SubmitTransferRingBuffer(handle->rxEdmaHandle, &xferConfig);
        EDMA_StartTransferRingBuffer(handle->rxEdmaHandle);
